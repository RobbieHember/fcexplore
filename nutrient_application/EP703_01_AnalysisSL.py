...
from fcexplore.nutrient_application import EP703_Util as utl
...
flg=1
if flg==0:
	sobs=utl.ImportData(meta)
else:
	dSite=gu.ReadExcel(meta['Paths']['Project'] + '\\EP703 Plot Level Summary.xlsx')
	sobs=gu.ipickle(meta['Paths']['Project'] + '\\Processed\\EP703_SL.pkl')
...
sobs['Csw_Net_RGR']=(np.log(sobs['Csw_t1'])-np.log(sobs['Csw_t0']))/sobs['DT']

# Initial biomass
u=np.unique(np.column_stack((sobs['ID_Site'],sobs['ID_Plot'])).astype(float),axis=0)
sobs['Csw_Init']=np.nan*np.ones(sobs['Csw_t0'].size)
for i in range(u.shape[0]):
	ind=np.where( (sobs['ID_Site']==u[i,0]) & (sobs['ID_Plot']==u[i,1]) )[0]
	atsf=np.abs(sobs['TSF_t0'][ind])
	if ind.size!=0:
		ind2=np.where( atsf==np.min(atsf) )[0]
		sobs['Csw_Init'][ind]=sobs['Csw_t0'][ind[ind2[0]]]

# Relative change in stand density (%/yr)
sobs['dN_rel']=sobs['N_Net']/sobs['N_t0']*100

sobs['RGR']=(np.log(sobs['Csw_t1'])-np.log(sobs['Csw_t0']))/sobs['DT']

sobs['Csw_G_ave']=sobs['Csw_G']/sobs['N_t1']*1000

#%%

ind=np.where( (sobs['ID_Site']==22) & (sobs['ID_Plot']==3) )[0]
plt.close('all'); plt.plot(sobs['TSF_t0'][ind],sobs['N_t0'][ind],'-bo')

#%%

spc='HW'
tl=np.array([0,20,35,50])
y=np.zeros(tl.size)
for i in range(tl.size):
	ND=0
	ind=np.where( (sobs['Spc1_CD_t0']==spc) & (sobs['NA_Dose']==ND) & (sobs['Thin_Level']==tl[i]) & (sobs['TSF_t0']>=0) & (sobs['TSF_t0']<=20) )[0]
	mu1=np.mean(sobs['Csw_G'][ind])
	ND=225
	ind=np.where( (sobs['Spc1_CD_t0']==spc) & (sobs['NA_Dose']==ND) & (sobs['Thin_Level']==tl[i]) & (sobs['TSF_t0']>=0) & (sobs['TSF_t0']<=20) )[0]
	mu2=np.mean(sobs['Csw_G'][ind])
	#y[i]=(mu2-mu1)/mu1*100
	y[i]=mu2-mu1
	#print(mu2-mu1)
plt.close('all'); plt.plot(tl,y,'-go')

#%%
spc='FD'
ND=0
TL=20
ind=np.where( (sobs['Spc1_CD_t0']==spc) & (sobs['NA_Dose']==ND) & (sobs['Thin_Level']==TL) )[0]
u=np.unique(sobs['TSF_t0'][ind])
y=np.zeros(u.size)
for i in range(u.size):
	ind=np.where( (sobs['Spc1_CD_t0']==spc) & (sobs['NA_Dose']==ND) & (sobs['Thin_Level']==TL) & (sobs['TSF_t0']==u[i]) )[0]
	y[i]=np.mean(sobs['Csw_M'][ind])
plt.close('all'); plt.plot(u,y,'-ro')

#%% Descriptive statistics

# Number of installations and C-F plot pairs (FD leading)
ind=np.where( (sobs['Spc1_CD_t0']=='FD') & (sobs['NA_Dose']==0) & (sobs['TSF_t0']==0) )[0]
...
ind=np.where( (sobs['Spc1_CD_t0']=='HW') & (sobs['NA_Dose']==0) & (sobs['TSF_t0']==0) )[0]
np.unique(sobs['ID_Site'][ind]).size
np.unique(np.column_stack((sobs['ID_Site'][ind],sobs['ID_Plot'][ind])),axis=0).size

# Biomass by pool
ind=np.where( (sobs['Spc1_CD_t0']=='FD') & (sobs['NA_Dose']==0) )[0]
np.nanmean(sobs['Csw_t0'][ind])
np.nanmean(sobs['Cbk_t0'][ind])
np.nanmean(sobs['Cbr_t0'][ind])
np.nanmean(sobs['Cf_t0'][ind])

# Get initial stand attributes at plot/treatment level
uDose=np.unique(sobs['NA_Dose'])

d={}
vs=['Spc1_CD_t0','Spc2_ID_t0','Spc1_Pct_t0','Spc2_Pct_t0','NA_Dose','SI','Age_t0','N_t0','Dam_t0','H_obs_t0','H_gf_t0','Csw_t0']
...
for iSite in range(uSite.size):
	...
	for iDose in range(uDose.size):
		ind=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['NA_Dose']==uDose[iDose]) )[0]
		ind=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['NA_Dose']==uDose[iDose]) & (sobs['TSF_t0_adj']==0) )[0]
...
iFDC=np.where( (d['Spc1_CD_t0']=='FD') & (d['NA_Dose']==0) )[0]
iFD225=np.where( (d['Spc1_CD_t0']=='FD') & (d['NA_Dose']==225) )[0]
iFD450=np.where( (d['Spc1_CD_t0']=='FD') & (d['NA_Dose']==450) )[0]
iFD675=np.where( (d['Spc1_CD_t0']=='FD') & (d['NA_Dose']==675) )[0]
iHWC=np.where( (d['Spc1_CD_t0']=='HW') & (d['NA_Dose']==0) )[0]
iHW225=np.where( (d['Spc1_CD_t0']=='HW') & (d['NA_Dose']==225) )[0]
iHW450=np.where( (d['Spc1_CD_t0']=='HW') & (d['NA_Dose']==450) )[0]
iHW675=np.where( (d['Spc1_CD_t0']=='HW') & (d['NA_Dose']==675) )[0]
for k in d.keys():
	if (k=='Spc1_CD_t0') | (k=='Spc2_ID_t0'): continue
...
iC0=np.where( (sobs['Spc1_CD_t0']==spc) & (sobs['TSF_t0']==-1) & (sobs['NA_Dose']==0) )[0]
iF0=np.where( (sobs['Spc1_CD_t0']==spc) & (sobs['TSF_t0']==-1) & (sobs['NA_Dose']==dose) )[0]
...
Csw_t0_C=np.nan*np.ones(id_t0_C.shape[0])
for i in range(id_t0_C.shape[0]):
	ind=np.where( (sobs['ID_Site']==id_t0_C[i,0]) & (sobs['ID_Plot']==id_t0_C[i,1]) & (sobs['TSF_t0']==-1) & (sobs['NA_Dose']==0) )[0]
	Csw_t0_C[i]=np.nanmean(sobs['Csw_t0'][ind])
Csw_t0_F=np.nan*np.ones(id_t0_F.shape[0])
for i in range(id_t0_F.shape[0]):
	ind=np.where( (sobs['ID_Site']==id_t0_F[i,0]) & (sobs['ID_Plot']==id_t0_F[i,1]) & (sobs['TSF_t0']==-1) & (sobs['NA_Dose']>0) )[0]
	Csw_t0_F[i]=np.nanmean(sobs['Csw_t0'][ind])

Csw_t1_C=np.nan*np.ones(id_t0_C.shape[0])
for i in range(id_t0_C.shape[0]):
	ind=np.where( (sobs['ID_Site']==id_t0_C[i,0]) & (sobs['ID_Plot']==id_t0_C[i,1]) & (sobs['TSF_t0']==0) & (sobs['NA_Dose']==0) )[0]
	Csw_t1_C[i]=np.nanmean(sobs['Csw_t0'][ind])
Csw_t1_F=np.nan*np.ones(id_t0_F.shape[0])
for i in range(id_t0_F.shape[0]):
	ind=np.where( (sobs['ID_Site']==id_t0_F[i,0]) & (sobs['ID_Plot']==id_t0_F[i,1]) & (sobs['TSF_t0']==0) & (sobs['NA_Dose']>0) )[0]
	Csw_t1_F[i]=np.nanmean(sobs['Csw_t0'][ind])

Gnet_C=Csw_t1_C-Csw_t0_C
Gnet_F=Csw_t1_F-Csw_t0_F
...
#%% Nitrogen response of biomass fluxes vs. time since N application
#uDose=np.unique(sobs['NA_Dose'])
#uDose=uDose[1:]
		...
		for iDose in range(uDose.size):
			...
			for iSite in range(uSite.size):
				for iTSF in range(uTSF.shape[0]):
					iC=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==uSpc[iSpc]) & (sobs['TSF_t0_adj']>=uTSF[iTSF,0]) & (sobs['TSF_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==0) )[0]
					iF=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==uSpc[iSpc]) & (sobs['TSF_t0_adj']>=uTSF[iTSF,0]) & (sobs['TSF_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==uDose[iDose]) )[0]
...
#plt.plot(rsT['FD']['Csw_Net'][225]['DA_mu'],'ob-')
...
ax[0,0].fill_between(tsf,rsT[spc]['Csw_G'][225]['DA_mu']-rsT[spc]['Csw_G'][225]['DA_se'],
  rsT[spc]['Csw_G'][225]['DA_mu']+rsT[spc]['Csw_G'][225]['DA_se'],color=[0.27,0.49,0.79],alpha=Alpha,linewidth=0)
ax[0,0].plot(tsf,rsT[spc]['Csw_G'][225]['DA_mu'],'-o',color=[0.27,0.49,0.79],markersize=ms,label='225 kg N ha$^-$$^1$')
ax[0,0].fill_between(tsf,rsT[spc]['Csw_G'][450]['DA_mu']-rsT[spc]['Csw_G'][450]['DA_se'],
  rsT[spc]['Csw_G'][450]['DA_mu']+rsT[spc]['Csw_G'][450]['DA_se'],color=[0,0.7,0],alpha=Alpha,linewidth=0)
ax[0,0].plot(tsf,rsT[spc]['Csw_G'][450]['DA_mu'],'-s',color=[0,0.7,0],markersize=ms,label='450 kg N ha$^-$$^1$')
ax[0,0].fill_between(tsf,rsT[spc]['Csw_G'][675]['DA_mu']-rsT[spc]['Csw_G'][675]['DA_se'],
  rsT[spc]['Csw_G'][675]['DA_mu']+rsT[spc]['Csw_G'][675]['DA_se'],color=[0.5,0,1],alpha=Alpha,linewidth=0)
ax[0,0].plot(tsf,rsT[spc]['Csw_G'][675]['DA_mu'],'-d',color=[0.5,0,1],markersize=ms,label='675 kg N ha$^-$$^1$')
...
ax[1,0].fill_between(tsf,np.cumsum(rsT[spc]['Csw_G'][225]['DA_mu']-rsT[spc]['Csw_G'][225]['DA_se']),
  np.cumsum(rsT[spc]['Csw_G'][225]['DA_mu']+rsT[spc]['Csw_G'][225]['DA_se']),color=[0.27,0.49,0.79],alpha=Alpha,linewidth=0)
ax[1,0].plot(tsf,np.cumsum(rsT[spc]['Csw_G'][225]['DA_mu']),'-o',color=[0.27,0.49,0.79],markersize=ms)
ax[1,0].fill_between(tsf,np.cumsum(rsT[spc]['Csw_G'][450]['DA_mu']-rsT[spc]['Csw_G'][450]['DA_se']),
  np.cumsum(rsT[spc]['Csw_G'][450]['DA_mu']+rsT[spc]['Csw_G'][450]['DA_se']),color=[0,0.7,0],alpha=Alpha,linewidth=0)
ax[1,0].plot(tsf,np.cumsum(rsT[spc]['Csw_G'][450]['DA_mu']),'-s',color=[0,0.7,0],markersize=ms)
ax[1,0].fill_between(tsf,np.cumsum(rsT[spc]['Csw_G'][675]['DA_mu']-rsT[spc]['Csw_G'][675]['DA_se']),
  np.cumsum(rsT[spc]['Csw_G'][675]['DA_mu']+rsT[spc]['Csw_G'][675]['DA_se']),color=[0.5,0,1],alpha=Alpha,linewidth=0)
ax[1,0].plot(tsf,np.cumsum(rsT[spc]['Csw_G'][675]['DA_mu']),'-d',color=[0.5,0,1],markersize=ms)
...
ax[0,1].fill_between(tsf,rsT[spc]['Csw_M'][225]['DA_mu']-rsT[spc]['Csw_M'][225]['DA_se'],
  rsT[spc]['Csw_M'][225]['DA_mu']+rsT[spc]['Csw_M'][225]['DA_se'],color=[0.27,0.49,0.79],alpha=Alpha,linewidth=0)
ax[0,1].plot(tsf,rsT[spc]['Csw_M'][225]['DA_mu'],'-o',color=[0.27,0.49,0.79],markersize=ms)
ax[0,1].fill_between(tsf,rsT[spc]['Csw_M'][450]['DA_mu']-rsT[spc]['Csw_M'][450]['DA_se'],
  rsT[spc]['Csw_M'][450]['DA_mu']+rsT[spc]['Csw_M'][450]['DA_se'],color=[0,0.7,0],alpha=Alpha,linewidth=0)
ax[0,1].plot(tsf,rsT[spc]['Csw_M'][450]['DA_mu'],'-s',color=[0,0.7,0],markersize=ms)
ax[0,1].fill_between(tsf,rsT[spc]['Csw_M'][675]['DA_mu']-rsT[spc]['Csw_M'][675]['DA_se'],
  rsT[spc]['Csw_M'][675]['DA_mu']+rsT[spc]['Csw_M'][675]['DA_se'],color=[0.5,0,1],alpha=Alpha,linewidth=0)
ax[0,1].plot(tsf,rsT[spc]['Csw_M'][675]['DA_mu'],'-d',color=[0.5,0,1],markersize=ms)
...
ax[1,1].fill_between(tsf,np.cumsum(rsT[spc]['Csw_M'][225]['DA_mu']-rsT[spc]['Csw_M'][225]['DA_se']),
  np.cumsum(rsT[spc]['Csw_M'][225]['DA_mu']+rsT[spc]['Csw_M'][225]['DA_se']),color=[0.27,0.49,0.79],alpha=Alpha,linewidth=0)
ax[1,1].plot(tsf,np.cumsum(rsT[spc]['Csw_M'][225]['DA_mu']),'-o',color=[0.27,0.49,0.79],markersize=ms)
ax[1,1].fill_between(tsf,np.cumsum(rsT[spc]['Csw_M'][450]['DA_mu']-rsT[spc]['Csw_M'][450]['DA_se']),
  np.cumsum(rsT[spc]['Csw_M'][450]['DA_mu']+rsT[spc]['Csw_M'][450]['DA_se']),color=[0,0.7,0],alpha=Alpha,linewidth=0)
ax[1,1].plot(tsf,np.cumsum(rsT[spc]['Csw_M'][450]['DA_mu']),'-s',color=[0,0.7,0],markersize=ms)
ax[1,1].fill_between(tsf,np.cumsum(rsT[spc]['Csw_M'][675]['DA_mu']-rsT[spc]['Csw_M'][675]['DA_se']),
  np.cumsum(rsT[spc]['Csw_M'][675]['DA_mu']+rsT[spc]['Csw_M'][675]['DA_se']),color=[0.5,0,1],alpha=Alpha,linewidth=0)
ax[1,1].plot(tsf,np.cumsum(rsT[spc]['Csw_M'][675]['DA_mu']),'-d',color=[0.5,0,1],markersize=ms)
...
for iSpc in range(len(uSpc)):
	...
	for iVar in range(len(uVar)):
		...
		for iDose in range(uDose.size):
			...
			for iSite in range(uSite.size):
				for iTSF in range(uTSF.shape[0]):
					iC=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==uSpc[iSpc]) & (sobs['Year_t0']>=uTSF[iTSF,0]) & (sobs['Year_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==0) )[0]
					iF=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==uSpc[iSpc]) & (sobs['Year_t0']>=uTSF[iTSF,0]) & (sobs['Year_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==uDose[iDose]) )[0]
					if (iC.size==0) | (iF.size==0): continue
					it=np.where( (tsf>=uTSF[iTSF,0]) & (tsf<=uTSF[iTSF,1]) )[0]
					Y_C_mu[it,iSite]=np.nanmean(sobs[uVar[iVar]][iC])
					Y_F_mu[it,iSite]=np.nanmean(sobs[uVar[iVar]][iF])
...
uVar=['Csw_G','Csw_M','Csw_Net','N_t0','Csw_t0','H_gf_t0','BA_t0']
...
for iVar in range(len(uVar)):
	...
	for iDose in range(uDose.size):
		...
		for iSite in range(uSite.size):
			for iTSF in range(uTSF.shape[0]):
				iC=np.where( (sobs['Spc1_CD_t0']!='X') & (sobs['ID_Site']==uSite[iSite]) & (sobs['TSF_t0_adj']>=uTSF[iTSF,0]) & (sobs['TSF_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==0) )[0]
				iF=np.where( (sobs['Spc1_CD_t0']!='X') & (sobs['ID_Site']==uSite[iSite]) & (sobs['TSF_t0_adj']>=uTSF[iTSF,0]) & (sobs['TSF_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==uDose[iDose]) )[0]
				if (iC.size==0) | (iF.size==0): continue
				it=np.where( (tsf>=uTSF[iTSF,0]) & (tsf<=uTSF[iTSF,1]) )[0]
				Y_C_mu[it,iSite]=np.nanmean(sobs[uVar[iVar]][iC])
				Y_F_mu[it,iSite]=np.nanmean(sobs[uVar[iVar]][iF])
...
for i in range(len(LeadSpc)):
	...
	ind=np.where(np.isnan(rsT['Csw_t0'][Dose]['yF_mu'][:,i])==False)[0]
	try:
		d=rsT['Csw_t0'][Dose]['yF_mu'][ind[0],i]-rsT['Csw_t0'][Dose]['yC_mu'][ind[0],i]
	except:
		d=0
	d=0
	ax[1,0].plot(tsf,rsT['Csw_t0'][Dose]['yC_mu'][:,i],'-b.')
	ax[1,0].plot(tsf,rsT['Csw_t0'][Dose]['yF_mu'][:,i]-d,'-g.')
	ax[1,0].set(ylabel='Stemwood biomass (MgC/ha)')

	ax[1,1].plot(tsf,rsT['Csw_G'][Dose]['yC_mu'][:,i],'-b.')
	ax[1,1].plot(tsf,rsT['Csw_G'][Dose]['yF_mu'][:,i],'-g.')
	ax[1,1].set(ylabel='Stemwood gross growth (MgC/ha/yr)')
	ax[1,2].plot(tsf,rsT['Csw_Net'][Dose]['yC_mu'][:,i],'-b.')
	ax[1,2].plot(tsf,rsT['Csw_Net'][Dose]['yF_mu'][:,i],'-g.')
	ax[1,2].set(ylabel='Stemwood net growth (MgC/ha/yr)')

	ax[2,0].plot(tsf,0*np.ones(tsf.size),'-k')
	ax[2,0].plot(tsf,rsT['Csw_G'][Dose]['yF_mu'][:,i]-rsT['Csw_G'][Dose]['yC_mu'][:,i],'-b.')
	ax[2,0].set(ylabel='Delta gross growth (MgC/ha/yr)')

	ax[2,1].plot(tsf,0*np.ones(tsf.size),'-k')
	ax[2,1].plot(tsf,rsT['Csw_M'][Dose]['yF_mu'][:,i]-rsT['Csw_M'][Dose]['yC_mu'][:,i],'-b.')
	ax[2,1].set(ylabel='Delta mortality (MgC/ha/yr)')

	ax[2,2].plot(tsf,0*np.ones(tsf.size),'-k')
	ax[2,2].plot(tsf,rsT['Csw_Net'][Dose]['yF_mu'][:,i]-rsT['Csw_Net'][Dose]['yC_mu'][:,i],'-b.')
...
for i in range(len(LeadSpc)):
	df.loc[i,'dB']=np.nanmean(rsT['Csw_t0'][Dose]['yF_mu'][0:2,i]-rsT['Csw_t0'][Dose]['yC_mu'][0:2,i])
	df.loc[i,'rdB']=np.nanmean( (rsT['Csw_t0'][Dose]['yF_mu'][0:2,i]-rsT['Csw_t0'][Dose]['yC_mu'][0:2,i])/rsT['Csw_t0'][Dose]['yC_mu'][0:2,i]*100 )

	df.loc[i,'dG']=np.nanmean(rsT['Csw_G'][Dose]['yF_mu'][iTSF,i]-rsT['Csw_G'][Dose]['yC_mu'][iTSF,i])
	df.loc[i,'dNet']=np.nanmean(rsT['Csw_Net'][Dose]['yF_mu'][iTSF,i]-rsT['Csw_Net'][Dose]['yC_mu'][iTSF,i])

	df.loc[i,'rdG']=np.nanmean( (rsT['Csw_G'][Dose]['yF_mu'][iTSF,i]-rsT['Csw_G'][Dose]['yC_mu'][iTSF,i])/rsT['Csw_G'][Dose]['yC_mu'][iTSF,i]*100 )
	df.loc[i,'rdNet']=np.nanmean( (rsT['Csw_Net'][Dose]['yF_mu'][iTSF,i]-rsT['Csw_Net'][Dose]['yC_mu'][iTSF,i])/rsT['Csw_Net'][Dose]['yC_mu'][iTSF,i]*100 )
...
y=rsT['Csw_M'][450]['DA_mu']
m,n=y.shape


dose=225
#iSpc=np.where( (LeadSpc==1) & (np.isnan(rsT['Csw_Net'][dose]['yC_mu'][40,:])==False) )[0]
iSpc=np.where( (LeadSpc>=0) )[0]
plt.close('all')
y=np.nanmean(rsT['Csw_G'][dose]['DA_mu'][:,iSpc],axis=1)
plt.plot(tsf,y,'-o',linewidth=2)
y=np.nanmean(rsT['Csw_M'][dose]['DA_mu'][:,iSpc],axis=1)
plt.plot(tsf,y,'-o',linewidth=2)
y=np.nanmean(rsT['Csw_Net'][dose]['DA_mu'][:,iSpc],axis=1)
plt.plot(tsf,y,'-o',linewidth=2,color=[0,0,0])


dose=225
ikp=np.where( (np.isnan(rsT['Csw_Net'][dose]['DA_mu'][40,:])==False) )[0]
plt.close('all')
y=np.nanmean(rsT['Csw_G'][dose]['DA_mu'][:,ikp],axis=1)
plt.plot(y,linewidth=2)
y=np.nanmean(rsT['Csw_M'][dose]['DA_mu'][:,ikp],axis=1)
plt.plot(y,linewidth=2)
y=np.nanmean(rsT['Csw_Net'][dose]['DA_mu'][:,ikp],axis=1)
plt.plot(y,linewidth=2,color=[0,0,0])


plt.close('all')
y=np.nan_to_num(np.nanmean(rsT['Csw_G'][dose]['DA_mu'][:,ikp],axis=1))
plt.plot(np.cumsum(y),linewidth=2)
y=np.nan_to_num(np.nanmean(rsT['Csw_M'][dose]['DA_mu'][:,ikp],axis=1))
plt.plot(np.cumsum(y),linewidth=2)
y=np.nan_to_num(np.nanmean(rsT['Csw_Net'][dose]['DA_mu'][:,ikp],axis=1))
plt.plot(np.cumsum(y),linewidth=2,color=[0,0,0])


y=np.nan_to_num(np.nanmean(rsT['Csw_G'][dose]['DA_mu'][:,iSpc],axis=1))-np.nan_to_num(np.nanmean(rsT['Csw_M'][dose]['DA_mu'][:,iSpc],axis=1))
...
spc=['FD','HW']
vr=['Csw_G_cc1','Csw_G_cc2','Csw_G_cc3','Csw_G_cc4',
	'Csw_M_cc1','Csw_M_cc2','Csw_M_cc3','Csw_M_cc4',
	'Csw_Net_cc1','Csw_Net_cc2','Csw_Net_cc3','Csw_Net_cc4']
rsD={}
		...
		for iTSF in range(uTSF.size):
			...
			for iSite in range(uSite.size):
				...
				for iDose in range(uDose.shape[0]):
					iC=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==spc[iSpc]) & (sobs['TSF_t0_adj']>=1) & (sobs['TSF_t1']<=uTSF[iTSF]) & (sobs['NA_Dose']==0) )[0]
					iF=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==spc[iSpc]) & (sobs['TSF_t0_adj']>=1) & (sobs['TSF_t1']<=uTSF[iTSF]) & (sobs['NA_Dose']==uDose[iDose]) )[0]
					if (iC.size==0) | (iF.size==0): continue
...
for i in range(4):
	y.append(rsD[spc]['Csw_G_cc' + str(i+1)][iTSF]['DA_mu'][np.where(uDose==dose)[0]][0])
	ye.append(rsD[spc]['Csw_G_cc' + str(i+1)][iTSF]['DA_se'][np.where(uDose==dose)[0]][0])
...
spc='FD'; dose=225; y=[]; ye=[]
for i in range(4):
	y.append(rsD[spc]['Csw_M_cc' + str(i+1)][iTSF]['DA_mu'][np.where(uDose==dose)[0]][0])
	ye.append(rsD[spc]['Csw_M_cc' + str(i+1)][iTSF]['DA_se'][np.where(uDose==dose)[0]][0])
...
spc='FD'; dose=225; y=[]; ye=[]
for i in range(4):
	y.append(rsD[spc]['Csw_Net_cc' + str(i+1)][iTSF]['DA_mu'][np.where(uDose==dose)[0]][0])
	ye.append(rsD[spc]['Csw_Net_cc' + str(i+1)][iTSF]['DA_se'][np.where(uDose==dose)[0]][0])
...
for iSpc in range(len(spc)):
	...
	for iVar in range(len(vr)):
		...
		for iTSF in range(uTSF.shape[0]):
			...
			for iSite in range(uSite.size):
				for iDose in range(uDose.shape[0]):
					#iC=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==spc[iSpc]) & (sobs['TSF_t0']>=0) & (sobs['TSF_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==0) )[0]
					#iF=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==spc[iSpc]) & (sobs['TSF_t0']>=0) & (sobs['TSF_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==uDose[iDose]) )[0]
					iC=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==spc[iSpc]) & (sobs['TSF_t0_adj']>=1) & (sobs['TSF_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==0) )[0]
					iF=np.where( (sobs['ID_Site']==uSite[iSite]) & (sobs['Spc1_CD_t0']==spc[iSpc]) & (sobs['TSF_t0_adj']>=1) & (sobs['TSF_t1']<=uTSF[iTSF,1]) & (sobs['NA_Dose']==uDose[iDose]) )[0]
					if (iC.size==0) | (iF.size==0): continue
					yCmu0[iDose,iSite]=np.mean(sobs[vr[iVar]][iC])
					yFmu0[iDose,iSite]=np.mean(sobs[vr[iVar]][iF])
			# Exclude plots if they have no site-paired control/treatment
...
ax[0,0].errorbar(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_G'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl1)
ax[0,0].plot(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],'-o',markersize=ms,color=cl1,label='Gross growth')
ax[0,0].errorbar(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_Net'][0]['DA_se'],capsize=3,linestyle='',ecolor=cl2)
ax[0,0].plot(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],'-s',markersize=ms,color=cl2,label='Net growth')
#ax[0,0].errorbar(uDose,-rsD[spc]['Csw_M'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_M'][0]['DA_se'],capsize=3,linestyle='',ecolor=cl3)
#ax[0,0].plot(uDose,-rsD[spc]['Csw_M'][iTSF]['DA_mu'],'-v',markersize=ms,color=cl3,label='-Mortality')
...
ax[0,1].errorbar(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_G'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl1)
ax[0,1].plot(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],'-o',markersize=ms,color=cl1)
ax[0,1].errorbar(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_Net'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl2)
ax[0,1].plot(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],'-s',markersize=ms,color=cl2)
...
ax[0,2].errorbar(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_G'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl1)
ax[0,2].plot(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],'-o',markersize=ms,color=cl1)
ax[0,2].errorbar(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_Net'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl2)
ax[0,2].plot(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],'-s',markersize=ms,color=cl2)
ax[0,2].set(position=[0.72,0.555,0.27,0.43],xlim=[0,1000],xticks=uDose,ylim=[-0.5,2])
spc='HW'
iTSF=np.where(uTSF[:,0]==1)[0][0]
ax[1,0].plot([0,1000],[0,0],'k-')
ax[1,0].errorbar(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_G'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl1)
ax[1,0].plot(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],'-o',markersize=ms,color=cl1,label='Gross growth')
ax[1,0].errorbar(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_Net'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl2)
ax[1,0].plot(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],'-s',markersize=ms,color=cl2,label='Net growth')
...
ax[1,1].errorbar(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_G'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl1)
ax[1,1].plot(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],'-o',markersize=ms,color=cl1)
ax[1,1].errorbar(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_Net'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl2)
ax[1,1].plot(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],'-s',markersize=ms,color=cl2)
...
ax[1,2].errorbar(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_G'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl1)
ax[1,2].plot(uDose,rsD[spc]['Csw_G'][iTSF]['DA_mu'],'-o',markersize=ms,color=cl1)
ax[1,2].errorbar(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],yerr=rsD[spc]['Csw_Net'][iTSF]['DA_se'],capsize=3,linestyle='',ecolor=cl2)
ax[1,2].plot(uDose,rsD[spc]['Csw_Net'][iTSF]['DA_mu'],'-s',markersize=ms,color=cl2)
...
cols=['ID_Site','Spc1_CD_t0','LogN_C','LogN_F','LogS_C','LogS_F']
...
for i in range(uSite.size):
	...
	df0.loc[cnt,'Spc1_CD_t0']=sobs['Spc1_CD_t0'][ind[0]]

	iC=np.where( (sobs['ID_Site']==uSite[i]) & (sobs['TSF_t0']<=tsf0_max) & (sobs['NA_Dose']==0) )[0]
	iF=np.where( (sobs['ID_Site']==uSite[i]) & (sobs['TSF_t0']<=tsf0_max) & (sobs['NA_Dose']==dose) )[0]
	df0.loc[cnt,'LogN_C']=np.mean(np.log(sobs['N_t0'][iC]))
	df0.loc[cnt,'LogN_F']=np.mean(np.log(sobs['N_t0'][iF]))
	df0.loc[cnt,'LogS_C']=np.mean(np.log(sobs['Csw_t0'][iC]*1000/sobs['N_t0'][iC]))
	df0.loc[cnt,'LogS_F']=np.mean(np.log(sobs['Csw_t0'][iF]*1000/sobs['N_t0'][iF]))
	cnt=cnt+1
...
for i in range(uSite.size):
	...
	df1.loc[cnt,'Spc1_CD_t0']=sobs['Spc1_CD_t0'][ind[0]]

	iC=np.where( (sobs['ID_Site']==uSite[i]) & (sobs['TSF_t0']<=tsf0_max) & (sobs['NA_Dose']==0) )[0]
	iF=np.where( (sobs['ID_Site']==uSite[i]) & (sobs['TSF_t0']<=tsf0_max) & (sobs['NA_Dose']==dose) )[0]
	df1.loc[cnt,'LogN_C']=np.mean(np.log(sobs['N_t0'][iC]))
	df1.loc[cnt,'LogN_F']=np.mean(np.log(sobs['N_t0'][iF]))
	df1.loc[cnt,'LogS_C']=np.mean(np.log(sobs['Csw_t0'][iC]*1000/sobs['N_t0'][iC]))
	df1.loc[cnt,'LogS_F']=np.mean(np.log(sobs['Csw_t0'][iF]*1000/sobs['N_t0'][iF]))
	cnt=cnt+1
...
# Douglas-fir
ikp=np.where( (df0['Spc1_CD_t0']=='FD') & (np.abs(df0['LogN_C'])<20) & (np.abs(df0['LogN_F'])<20) & (df0['LogS_C']>1.8) )[0]
...
ikp2=np.where( (df1['Spc1_CD_t0']=='FD') & (np.abs(df1['LogN_C'])<20) & (np.abs(df1['LogN_F'])<20) & (df1['LogS_C']>1.8) )[0]
...
# Western hemlock
ikp=np.where( (df0['Spc1_CD_t0']=='HW') & (np.abs(df0['LogN_C'])<20) & (np.abs(df0['LogN_F'])<20) & (df0['LogS_C']>1.8) )[0]
...
ikp2=np.where( (df1['Spc1_CD_t0']=='HW') & (np.abs(df1['LogN_C'])<20) & (np.abs(df1['LogN_F'])<20) & (df1['LogS_C']>1.8) )[0]
mdF2=sm.RLM(df1.loc[ikp2,'LogN_F'].values,np.c_[np.ones(ikp2.size),df1.loc[ikp2,'LogS_F'].values]).fit()
mdF2.summary()
yhatF2=mdF2.predict(np.c_[np.ones(xhat.size),xhat])
...
plt.close('all');
fig,ax=plt.subplots(1,2,figsize=gu.cm2inch(15.5,7))
ikp=np.where( (sobs['Spc1_CD_t0']=='FD') & (sobs['NA_Dose']==0) )[0]
ax[0].plot(sobs['Csw_t0'][ikp]/sobs['N_t0'][ikp],sobs['N_t0'][ikp],'.')
ikp=np.where( (sobs['Spc1_CD_t0']=='FD') & (sobs['NA_Dose']==225) )[0]
ax[0].plot(sobs['Csw_t0'][ikp]/sobs['N_t0'][ikp],sobs['N_t0'][ikp],'.')
ax[0].set(yscale='log',xscale='log')
ikp=np.where( (sobs['Spc1_CD_t0']=='HW') & (sobs['NA_Dose']==0) )[0]
ax[1].plot(sobs['Csw_t0'][ikp]/sobs['N_t0'][ikp],sobs['N_t0'][ikp],'.')
ikp=np.where( (sobs['Spc1_CD_t0']=='HW') & (sobs['NA_Dose']==225) )[0]
ax[1].plot(sobs['Csw_t0'][ikp]/sobs['N_t0'][ikp],sobs['N_t0'][ikp],'.')
...
uSpc=['FD','HW']
uDose=[225,450]
uVar=['dN_rel','RGR','Age_t0','N_t0','N_Net','BA_t0','H_obs_t0','Csw_t0','Csw_Net','Csw_G','Csw_M']
rts={}
for iSpc in range(len(uSpc)):
	rts[uSpc[iSpc]]={}
	for iDose in range(len(uDose)):
		...
		for iVar in range(len(uVar)):
			...
			for iYr in range(sobs['Year_t0'].size):
				...
				if (sobs['Spc1_CD_t0'][iYr]==uSpc[iSpc]) & (sobs['NA_Dose'][iYr]==0):
					#it=np.where( (tv>=sobs['Year_t0'][iYr]) & (tv<=sobs['Year_t1'][iYr]) )[0]
					rts[uSpc[iSpc]][uDose[iDose]]['y_c'][uVar[iVar]][it,cnt_c]=sobs[uVar[iVar]][iYr];
					cnt_c=cnt_c+1
				if (sobs['Spc1_CD_t0'][iYr]==uSpc[iSpc]) & (sobs['NA_Dose'][iYr]==uDose[iDose]):
					#it=np.where( (tv>=sobs['Year_t0'][iYr]) & (tv<=sobs['Year_t1'][iYr]) )[0]
...
for i in range(len(da)):
	indC=np.where(sobs['NA_Dose']==0)[0]
	indF=np.where(sobs['NA_Dose']==225)[0]
...
for i in range(sobs['Year_t0'].size):
	...
	if (sobs['Spc1_CD_t0'][i]=='FD') & (sobs['NA_Dose'][i]==0):
		it=np.where( (tv>=sobs['Year_t0'][i]) & (tv<=sobs['Year_t1'][i]) )[0]
		Gc[it,c_c]=sobs['Csw_M'][i]
		Ac[it,c_c]=sobs['Age_t0'][i]
		c_c=c_c+1
	if (sobs['Spc1_CD_t0'][i]=='FD') & (sobs['NA_Dose'][i]>0) & (sobs['NA_Dose'][i]==225):
		it=np.where( (tv>=sobs['Year_t0'][i]) & (tv<=sobs['Year_t1'][i]) )[0]
		Gf1[it,c_f1]=sobs['Csw_M'][i]
		c_f1=c_f1+1
...
uSpc=['FD','HW']
uDose=[225,450]
uVar=['N_t0','N_Net','BA_t0','H_obs_t0','Csw_t0','Csw_Net','Csw_G','Csw_M']
rts={}
for iSpc in range(len(uSpc)):
	rts[uSpc[iSpc]]={}
	for iDose in range(len(uDose)):
...
uV=['N_t0','Dam_t0','H_obs_t0','H_gf_t0','Csw_t0','H_obs_G','Csw_Net']
uS=['FD','HW']
	...
	for iDose in range(len(uDose)):
		...
		for iV in range(len(uV)):
			...
			for j in range(uA.size):
				ind=np.where( (sobs['Age_t0']==uA[j]) & (sobs['Spc1_CD_t0']==uS[iS]) & (sobs['NA_Dose']==uDose[iDose]) )[0]
...
ax[3,0].fill_between(uA,fA_mu[0][0]['Csw_t0']-fA_sig[0][0]['Csw_t0'],
  fA_mu[0][0]['Csw_t0']+fA_sig[0][0]['Csw_t0'],color=[0.27,0.49,0.79],alpha=Alpha,linewidth=0)
ax[3,0].plot(uA,fA_mu[0][0]['Csw_t0'],'-o',color=[0.27,0.49,0.79],markersize=ms)
...
ax[3,1].fill_between(uA,fA_mu[1][0]['Csw_t0']-fA_sig[1][0]['Csw_t0'],
  fA_mu[1][0]['Csw_t0']+fA_sig[1][0]['Csw_t0'],color=[0.27,0.49,0.79],alpha=Alpha,linewidth=0)
ax[3,1].plot(uA,fA_mu[1][0]['Csw_t0'],'-o',color=[0.27,0.49,0.79],markersize=ms)
...
cols=['ID_Site','Spc1_CD_t0','Year_t0','TSF_t0','TSF_t1', \
...
for i in range(uSite.size):
	...
	for j in range(uTSF.shape[0]):
		...
		iC=np.where((sobs['ID_Site']==uSite[i]) & (sobs['TSF_t0']==uTSF[j,0]) & (sobs['NA_Dose']==0))[0]
		iF=np.where((sobs['ID_Site']==uSite[i]) & (sobs['TSF_t0']==uTSF[j,0]) & (sobs['NA_Dose']==dose))[0]
...
cols=['ID_Site','DA_mu','Age_Init','NA_Dose','Spc1_CD_t0','SI','N_Init','Csw_Init','pH_Min_Init','pH_Humus_Init','P_Min_Init','P_Humus_Init','Ca_Min_Init']#,'ndep_ann_r','tmean_ann_r','prcp_ann_r']
...
for i in range(uSite.size):
	...
	dfR.loc[cnt,'NA_Dose']=sobs['NA_Dose'][iF[0]]
...
dfR=dfR.drop(['ID_Site','Spc1_CD_t0'],axis=1)
...
# Predictor variables
xv=['SI','Age_Init','pH_Min_Init','NA_Dose']
xv=['Age_Init']
#xv=['NA_Dose','pH_Min_Init','P_Min_Init','Ca_Min_Init']
...
# SI
plt.close('all')
ind=np.where(dfR['Spc1_CD_t0']=='FD')[0]
plt.plot(dfR.loc[ind,'SI'],dfR.loc[ind,'DA_mu'],'o')
ind=np.where(dfR['Spc1_CD_t0']=='HW')[0]

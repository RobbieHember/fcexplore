...
#%% Project assumptions
wfss={'ByZone':{}}
wfss['tv_obs']=np.arange(1920,2024,1)
wfss['tv_scn']=np.arange(-2000,2201,1)
wfss['tv_long']=np.arange(1000,2024,1)
ivl=50
...
for iZ in range(bgcz_cd.size):
    zone=bgcz_cd[iZ]
    wfss['ByZone'][zone]={}
    wfss['ByZone'][zone]['Area Wildfire']=np.zeros(wfss['tv_obs'].size)
    ind=np.where( (zZ_Full==lut_bgcz[zone]) & (zLC_Full==lut_lc['Forest']) )[0]
    wfss['ByZone'][zone]['Area Zone']=ind.size
    ind=np.where( (zZ_Sub==lut_bgcz[zone]) & (zLC_Sub==lut_lc['Forest']) )[0]
    wfss['ByZone'][zone]['Occurrence']=np.zeros((wfss['tv_obs'].size,ind.size),dtype='int8')
...
for iY in range(6):
    ...
    for iZ in range(bgcz_cd.size):
        ...
        for yr in idxF.keys():
            if yr==0: continue
            iT=np.where(wfss['tv_obs']==yr)[0]
            if iT.size==0: continue
            ind=np.where( (zZ_Full[idxF[yr]]==lut_bgcz[zone]) & (zLC_Full[idxF[yr]]==lut_lc['Forest']) )[0]
            wfss['ByZone'][zone]['Area Wildfire'][iT]=wfss['ByZone'][zone]['Area Wildfire'][iT]+ind.size
        iZoneS=np.where( (zZ_Sub==lut_bgcz[zone]) & (zLC_Sub==lut_lc['Forest']) )[0]
        idxS=gu.IndicesFromUniqueArrayValues(zFire[0::ivl,0::ivl].flatten()[iZoneS])
        for yr in idxS.keys():
            if yr==0: continue
            iT=np.where(wfss['tv_obs']==yr)[0]
            if iT.size==0: continue                
            wfss['ByZone'][zone]['Occurrence'][iT,idxS[yr]]=1
    del zFire,idxF,idxS
    garc.collect()

#%% Calculate annual probability of occurrence
for zone in wfss['ByZone'].keys():
    wfss['ByZone'][zone]['Po_obs']=np.sum(wfss['ByZone'][zone]['Occurrence'])/wfss['ByZone'][zone]['Occurrence'].size
...
if flg==1:
    for zone in wfss['ByZone'].keys():
        d={}
        d['Occurrence']=wfss['ByZone'][zone]['Occurrence'].flatten()
        ...
        wfss['ByZone'][zone]['Po_obs2']=log_reg.predict(d2)

    plt.close('all')
    for zone in wfss['ByZone'].keys():
        plt.plot(wfss['ByZone'][zone]['Po_obs'],wfss['ByZone'][zone]['Po_obs2'],'ko')
...
flg=0
if flg==1:
    zone='BWBS'; wfss['ByZone'][zone]['Po_obs']=1.3*wfss['ByZone'][zone]['Po_obs']
    zone='PP'; wfss['ByZone'][zone]['Po_obs']=1.24*wfss['ByZone'][zone]['Po_obs']
    zone='SBPS'; wfss['ByZone'][zone]['Po_obs']=1.18*wfss['ByZone'][zone]['Po_obs']

    zone='CMA'; wfss['ByZone'][zone]['Po_obs']=0.74*wfss['ByZone'][zone]['Po_obs']
    zone='MH'; wfss['ByZone'][zone]['Po_obs']=0.7*wfss['ByZone'][zone]['Po_obs']

#%% Calculate total area burned by year
wfss['A_Burned_Tot']=np.zeros(wfss['tv_obs'].size); cnt=0
for zone in wfss['ByZone'].keys():
    wfss['A_Burned_Tot']=wfss['A_Burned_Tot']+wfss['ByZone'][zone]['Area Wildfire']

# plt.close('all'); fig,ax=plt.subplots(1,figsize=gu.cm2inch(12,5.5))
# ax.bar(wfss['tv_obs'],A_Burned/1e6,facecolor=[0.75,0.65,0.55])
...
wfss['Po_obs_mu']=np.zeros(len(wfss['ByZone'].keys())); cnt=0
for k in wfss['ByZone'].keys():
    wfss['Po_obs_mu'][cnt]=wfss['ByZone'][k]['Po_obs']; cnt=cnt+1

ord=np.flip(np.argsort(wfss['Po_obs_mu']))
wfss['bgcz_cdS']=bgcz_cd.copy()[ord]
wfss['Po_obs_mu']=wfss['Po_obs_mu'][ord]
...
#        xticklabels=wfss['bgcz_cdS'],xlabel='BGC zone',ylabel='Probability of occurrence (% yr$^-$$^1$)',xlim=[0.5,Po_obs_mu.size+.5],ylim=[0,0.5])
# ax.yaxis.set_ticks_position('both'); ax.xaxis.set_ticks_position('both'); ax.tick_params(length=1.5)
# plt.tight_layout()
...
cnt=0
for zone in wfss['ByZone'].keys():
    Ao=wfss['ByZone'][zone]['Area Wildfire']/wfss['ByZone'][zone]['Area Zone']
    shape,loc,scale=stats.pareto.fit(Ao)
    wfss['ByZone'][zone]['Beta_Pareto']=np.array([shape,loc,scale])
    #print(scale)
    dS['Shape'][cnt]=shape
    dS['Scale'][cnt]=scale
    dS['Mean Po (obs)'][cnt]=100*wfss['ByZone'][zone]['Po_obs']
...
for zone in wfss['ByZone'].keys():
    ...
    d=wfss['ByZone'][zone]['Po_obs']-Po_hat_mu
    iMin=np.where(np.abs(d)==np.min(np.abs(d)))[0]
    Po_hat_mu[iMin]
    Scale=bin[iMin[0]]
    wfss['ByZone'][zone]['Beta_Pareto_Cal']=np.array([Shape0,-Scale,Scale])
    #print(Scale)

#%% Plot time series of observed annual area burned (%/yr)

zone='SBS'

print(wfss['ByZone'][zone]['Beta_Pareto'])
#shape=Shape0; loc=-0.0085; scale=-loc;
#shape=wfss['ByZone'][zone]['Beta_Pareto'][0]; scale=-1*wfss['ByZone'][zone]['Beta_Pareto'][1]; loc=-wfss['ByZone'][zone]['Beta_Pareto'][2]
shape=wfss['ByZone'][zone]['Beta_Pareto_Cal'][0]; loc=wfss['ByZone'][zone]['Beta_Pareto_Cal'][1]; scale=wfss['ByZone'][zone]['Beta_Pareto_Cal'][2]

it0=np.where(wfss['tv_long']<1920)[0]

plt.close('all'); fig,ax=plt.subplots(5,1,figsize=gu.cm2inch(15,9))
ax[0].bar(wfss['tv_obs'],wfss['ByZone'][zone]['Area Wildfire']/wfss['ByZone'][zone]['Area Zone']*100,1,facecolor=[0.5,0,0]);
yhat=stats.pareto.rvs(shape,loc=loc,scale=scale,size=wfss['tv_long'].size);
ax[0].bar(wfss['tv_long'][it0],yhat[it0]*100,1,fc=[1,0.6,0]);
ax[0].set(xticks=np.arange(wfss['tv_long'][0],wfss['tv_obs'][-1]+2,100),xticklabels='',ylabel='',xlabel='',yticks=np.arange(0,12,2),xlim=[wfss['tv_long'][0]-0.5,wfss['tv_long'][-1]+1+0.5],ylim=[0,12]);
ax[0].yaxis.set_ticks_position('both'); ax[0].xaxis.set_ticks_position('both'); ax[0].tick_params(length=1.5)

ax[1].bar(wfss['tv_obs'],wfss['ByZone'][zone]['Area Wildfire']/wfss['ByZone'][zone]['Area Zone']*100,1,facecolor=[0.5,0,0]);
yhat=stats.pareto.rvs(shape,loc=loc,scale=scale,size=wfss['tv_long'].size);
ax[1].bar(wfss['tv_long'][it0],yhat[it0]*100,1,fc=[1,0.6,0]);
ax[1].set(xticks=np.arange(wfss['tv_long'][0],wfss['tv_obs'][-1]+2,100),xticklabels='',ylabel='',xlabel='',yticks=np.arange(0,12,2),xlim=[wfss['tv_long'][0]-0.5,wfss['tv_long'][-1]+1+0.5],ylim=[0,12]);
ax[1].yaxis.set_ticks_position('both'); ax[1].xaxis.set_ticks_position('both'); ax[1].tick_params(length=1.5)

ax[2].bar(wfss['tv_obs'],wfss['ByZone'][zone]['Area Wildfire']/wfss['ByZone'][zone]['Area Zone']*100,1,facecolor=[0.5,0,0]);
yhat=stats.pareto.rvs(shape,loc=loc,scale=scale,size=wfss['tv_long'].size);
ax[2].bar(wfss['tv_long'][it0],yhat[it0]*100,1,fc=[1,0.6,0]);
ax[2].set(xticks=np.arange(wfss['tv_long'][0],wfss['tv_obs'][-1]+2,100),xticklabels='',ylabel='Area burned (% yr$^-$$^1$)',xlabel='',yticks=np.arange(0,12,2),xlim=[wfss['tv_long'][0]-0.5,wfss['tv_long'][-1]+1+0.5],ylim=[0,12])
ax[2].yaxis.set_ticks_position('both'); ax[2].xaxis.set_ticks_position('both'); ax[2].tick_params(length=1.5)

ax[3].bar(wfss['tv_obs'],wfss['ByZone'][zone]['Area Wildfire']/wfss['ByZone'][zone]['Area Zone']*100,1,facecolor=[0.5,0,0]);
yhat=stats.pareto.rvs(shape,loc=loc,scale=scale,size=wfss['tv_long'].size);
ax[3].bar(wfss['tv_long'][it0],yhat[it0]*100,1,fc=[1,0.6,0]);
ax[3].set(xticks=np.arange(wfss['tv_long'][0],wfss['tv_obs'][-1]+2,100),xticklabels='',ylabel='',xlabel='',yticks=np.arange(0,12,2),xlim=[wfss['tv_long'][0]-0.5,wfss['tv_long'][-1]+1+0.5],ylim=[0,12]);
ax[3].yaxis.set_ticks_position('both'); ax[3].xaxis.set_ticks_position('both'); ax[3].tick_params(length=1.5)

ax[4].bar(wfss['tv_obs'],wfss['ByZone'][zone]['Area Wildfire']/wfss['ByZone'][zone]['Area Zone']*100,1,facecolor=[0.5,0,0]);
yhat=stats.pareto.rvs(shape,loc=loc,scale=scale,size=wfss['tv_long'].size);
ax[4].bar(wfss['tv_long'][it0],yhat[it0]*100,1,fc=[1,0.6,0]);
ax[4].set(xticks=np.arange(wfss['tv_long'][0],wfss['tv_obs'][-1]+2,100),ylabel='',xlabel='Time, years',yticks=np.arange(0,12,2),xlim=[wfss['tv_long'][0]-0.5,wfss['tv_long'][-1]+1+0.5],ylim=[0,12]);
...
c=0
for zone in wfss['ByZone'].keys():
    y[c]=wfss['ByZone'][zone]['Beta_Pareto_Cal'][2]
    x[c]=wfss['ByZone'][zone]['Po_obs']*100
    c=c+1
...
for zone in wfss['ByZone'].keys():
    wfss['ByZone'][zone]['Pareto_scale_to_match_Po_mu']=beta
...
dChar=gu.ReadExcel(r'C:\Data\DigitizedFigures\Marlonetal2012_Fig2.xlsx')
...
for zone in wfss['ByZone'].keys():
    ...
    # Observed Po
    Po_obs=wfss['ByZone'][zone]['Po_obs']*100

    # Indices
    iCharPre=np.where( (tvChar>=wfss['tv_scn'][0]) & (tvChar<=1919) )[0]
    iCharCalPre=np.where( (tvChar>=500)  & (tvChar<=1919) )[0]
    iCharCal=np.where( (tvChar>=1919) )[0]
    ind0=np.where( (tvChar>=tvChar[0]) & (tvChar<=1919) )[0]
    ind1=np.where( (wfss['tv_scn']>=tvChar[0])  & (wfss['tv_scn']<=1919) )[0]
    indF=np.where(wfss['tv_scn']>=1920)[0]
    indF2=np.where(wfss['tv_scn']>2020)[0]

    # Initialize
    dF={'F0':{},'F1':{},'F2':{},'F3':{}}
    wfss['ByZone'][zone]['Po Det Scenarios']={'H0':dF.copy(),'H1':dF.copy(),'H2':dF.copy(),'H3':dF.copy(),'H4':dF.copy()}

    #--------------------------------------------------------------------------
    # Constant Po from observations
    #--------------------------------------------------------------------------
    
    wfss['ByZone'][zone]['Po Det Scenarios']['H0']['F0']=1.0*Po_obs*np.ones(wfss['tv_scn'].size)

    dH={}
    dH['H0']=wfss['ByZone'][zone]['Po Det Scenarios']['H0']['F0'].copy()
    ...
    dH['H4']=dH['H2'].copy()
    ind4=np.where(wfss['tv_scn']>1888)[0]
    dH['H4'][ind4]=np.mean(dH['H2'][ind1])
    wfss['ByZone'][zone]['Po Det Scenarios']['H4']['F0']=dH['H4']
    ...
    if flg==1:
        ...
        ax.add_patch(rc)
        iT1=np.where(wfss['tv_scn']<1920)[0]; iT2=np.where( (wfss['tv_scn']>=1920) & (wfss['tv_scn']<=2024) )[0]
        ...
        print(pMu)
        print(pHi)
        plt.plot(wfss['tv_scn'],Po_obs*np.ones(wfss['tv_scn'].size),'k-',lw=0.5,label='H0: Constant observed')
        plt.plot(wfss['tv_scn'],dH['H1'],'b-',lw=lw,color=[0.27,0.49,0.77],label='H1: PI period = 1.5 x modern period')
        plt.plot(wfss['tv_scn'],dH['H2'],'g--',lw=lw,color=[1,0.45,0],label='H2: PI period = 2.0 x modern period')
        plt.plot(wfss['tv_scn'],dH['H3'],'r-.',lw=lw,color=[0.85,0,0],label='H3: PI period = 2.5 x modern period')
        plt.plot(wfss['tv_scn'],dH['H4'],'r:',lw=lw,color=[0.5,0,0.95],label='H4: Constant H2')
    ...
    for k in wfss['ByZone'][zone]['Po Det Scenarios']['H1'].keys():
        if k=='F0': continue
        wfss['ByZone'][zone]['Po Det Scenarios']['H1'][k]=dH['H1'].copy()
        wfss['ByZone'][zone]['Po Det Scenarios']['H2'][k]=dH['H2'].copy()
        wfss['ByZone'][zone]['Po Det Scenarios']['H3'][k]=dH['H3'].copy()
        wfss['ByZone'][zone]['Po Det Scenarios']['H4'][k]=dH['H4'].copy()
    ...
    for k in wfss['ByZone'][zone]['Po Det Scenarios'].keys():
        if k=='H0': continue
        wfss['ByZone'][zone]['Po Det Scenarios'][k]['F1']=dH[k].copy()
        wfss['ByZone'][zone]['Po Det Scenarios'][k]['F1'][indF2]=np.interp(wfss['tv_scn'][indF2],np.array([2020,2200]),np.array([Po_obs,5.5*Po_obs]))

    # 5 x observation mean by 2100
    for k in wfss['ByZone'][zone]['Po Det Scenarios'].keys():
        if k=='H0': continue
        wfss['ByZone'][zone]['Po Det Scenarios'][k]['F2']=dH[k].copy()        
        wfss['ByZone'][zone]['Po Det Scenarios'][k]['F2'][indF2]=np.interp(wfss['tv_scn'][indF2],np.array([2020,2200]),np.array([Po_obs,10*Po_obs]))

    # 20 x observation mean by 2100
    for k in wfss['ByZone'][zone]['Po Det Scenarios'].keys():
        if k=='H0': continue
        wfss['ByZone'][zone]['Po Det Scenarios'][k]['F3']=dH[k].copy()
        wfss['ByZone'][zone]['Po Det Scenarios'][k]['F3'][indF2]=np.interp(wfss['tv_scn'][indF2],np.array([2020,2200]),np.array([Po_obs,43.75*Po_obs]))
...
if flg==1:
    for zone in wfss['ByZone'].keys():
        del wfss['ByZone'][zone]['Occurrence']

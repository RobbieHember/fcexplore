...
#%% Basic Pareto distribution of annual area affected
#d=gu.ipickle(meta['Paths']['Model']['Taz Datasets'] + '\\Wildfire Stats and Scenarios\Wildfire_Stats_Scenarios_By_BGCZ.pkl')

plt.close('all'); fig,ax=plt.subplots(1,figsize=gu.cm2inch(15,8.5)); 
b=1.5; x=np.linspace(stats.pareto.ppf(0.01,b),stats.pareto.ppf(0.99,b),100)
ax.plot(x,stats.pareto.pdf(x,b),'b-',lw=1, alpha=0.6, label='pareto pdf')
b=2; x=np.linspace(stats.pareto.ppf(0.01,b),stats.pareto.ppf(0.99,b),100)
ax.plot(x,stats.pareto.pdf(x,b),'g--',lw=1, alpha=0.6, label='pareto pdf')
b=4; x=np.linspace(stats.pareto.ppf(0.01,b),stats.pareto.ppf(0.99,b),100)
ax.plot(x,stats.pareto.pdf(x,b),'r-.',lw=1, alpha=0.6, label='pareto pdf')

mean,var,skew,kurt=stats.pareto.stats(1.5,moments='mvsk')
mean

#%%
beta=[1.6,-0.001,0.001]
N_t=1000
tv=np.arange(0,N_t,1)
Po=stats.pareto.rvs(beta[0],loc=beta[1],scale=beta[2],size=N_t)
plt.close('all'); plt.bar(tv,Po*100)
print(np.median(A_Affected)*100)

#plt.hist(A_Affected)

#%% EVERTYING BELOW HERE IS OUT OF DATE AND RETIRED
...
#     ibmss[k]={}
#     ibmss[k]['Ao']=np.zeros(tv_obs.size)
#     ind=np.where( (z['bgcz']['Data']==meta['LUT']['BEC_BIOGEOCLIMATIC_POLY']['ZONE'][k]) )
#     ibmss[k]['Oc']=np.zeros((tv_obs.size,ind[0].size))
#     ibmss[k]['Azone']=ind[0].size

# for iT in range(tv_obs.size):
#     print(tv_obs[iT])
#     zDist=gis.OpenGeoTiff(meta['Paths']['bc1ha'] + '\\PEST_INFESTATION_POLY\\PEST_SEVERITY_CODE_IBM_' + str(tv_obs[iT]) + '.tif')

...
ind=np.where( (zHRE['Data']>0) & (zFCR['Data']==0) & (zCrown['Data']>0) ); z[ind]=1
ind=np.where( (zHCC['Data']>0) & (zFCR['Data']==0) & (zCrown['Data']>0) ); z[ind]=2
plt.matshow(z)
...
for iT in range(d['tv'].size):
    ...
    ind=np.where( (zCrown['Data']>0) & (zFCR['Data']==0) & (zHy['Data']>0) )
    d['A Harvest'][iT]=ind[0].size

    ind=np.where( (zCrown['Data']>0) & (zFCR['Data']==0) & (zHCCy['Data']>0) )
    d['A Harvest CC'][iT]=ind[0].size

    ind=np.where( (zCrown['Data']>0) & (zHy['Data']>0) & (zFCR['Data']==0) & (zPL['Data']>0) )
...
d2=gu.ipickle(r'C:\Users\rhember\Documents\Data\Harvest\Harvest Area\HarvestAreaBC.pkl')
...
plt.plot(d2['tv'],d2['Area Harvested CC']/1e3,'-cd')

fig,ax=plt.subplots(1,figsize=gu.cm2inch(12,12));
plt.plot(d['tv'],d['A Harvest and Planted']/d['A Harvest'],'-ko')
